    ...
    <header className="sticky  w-full top-0 z-50 bg-transparent">
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden md:flex md:items-center md:gap-4">
          ...
          <Link
            href="/signup"
            className=" bg-[#4640DE] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#4844c7] transition-colors"
          >
            Sign Up
          </Link>
        </div>
      ...
      {/* Slide-in panel from right, 60% width */}
      <div
        className={`md:hidden fixed top-[64px] right-0 h-[calc(100vh-64px)] w-[60%] bg-[#f5f7fa] shadow-2xl
      ...
      >
        <div className="flex flex-col gap-1 px-6 py-6">
          ...
          <div className="flex gap-3 pt-6">
            <Link
              href="/login"
              className="text-center py-2.5 px-3 text-[#4640DE] font-semibold border border-[#5a56e9] hover:bg-[#5a56e9] w-full hover:text-white transition-colors"
              onClick={() => setMobileMenuOpen(false)}
            >
              Login
            </Link>
            <Link
              href="/signup"
              className="text-center  bg-[#4640DE] px-3 w-full py-2.5 font-semibold text-white hover:bg-[#4844c7] transition-colors"
              onClick={() => setMobileMenuOpen(false)}
            >
              Sign Up
            </Link>
